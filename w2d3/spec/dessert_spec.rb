...
describe Dessert do
  let(:chef) { double("chef", name: "joyce") }
  subject(:dessert) { Dessert.new("cake", 10, chef) }
  subject(:dessert2) { Dessert.new("cake", "20", chef) }
  ...
  describe "#mix!" do
    it "shuffles the ingredient array" do
      ...
      expect(dessert.ingredients.dup).to_not eq(dessert.mix!)
    end
  end
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      allow(chef).to receive(:titleize).and_return("Chef Joyce the Great Baker")
      expect(dessert.serve).to include("Joyce")
    end
  end
